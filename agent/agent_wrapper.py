...
class WebAgentV3:
    ...
    async def submit_task(self, task: str, auto_confirm: bool = False) -> str:
        ...
        asyncio.create_task(self._run_in_background(task, auto_confirm, task_id))
        return task_id

    async def _run_in_background(self, task: str, auto_confirm: bool, task_id: str):
        ...
        try:
            ...
            await self._bus.publish("error", {"code": "agent_error", "message": str(e)})
        finally:
    ...
    def _encode_image_b64(self, image: np.ndarray, quality: int = 85) -> str:
        ...
        return base64.b64encode(self._encode_image_bytes(image, quality)).decode(
            "utf-8"
        )
    ...
    def _wait_for_confirmation(self, action_str: str) -> bool:
        ...
        self._publish(
            "agent.state_changed", {"state": "awaiting_confirm", "action": action_str}
        )
        ...
        self._state = "running"
        self._publish(
            "agent.state_changed", {"state": "running", "task": self._current_task}
        )
        return result
    ...
    def _execute_look(self, args: dict) -> tuple[str, list[dict], list[str]]:
        ...
        image_blocks = [
            ...
            {
                "type": "text",
                "text": "[Camera image with coordinate grid. Depth image below.]",
            },
    ...
    def _execute_detect(self, args: dict) -> tuple[str, list[dict], list[str]]:
        ...
        if self._hw.gemini_vision is None:
            return (
                json.dumps(
                    {"status": "error", "message": "Gemini vision not available."}
                ),
                [],
                [],
            )
    ...
    def _execute_segment(self, args: dict) -> tuple[str, list[dict], list[str]]:
        ...
        if self._hw.sam2 is None:
            return (
                json.dumps({"status": "error", "message": "SAM2 not available."}),
                [],
                [],
            )
        ...
        # Compute 3D arm coordinates
        arm_coords = None
        depth_mm = self._hw.ct.get_depth_at_pixel(
            self._hw.vision_depth_frame, pixel_x, pixel_y
        )
        if depth_mm is not None:
    ...
    def _execute_goto_pixel(self, args: dict) -> dict:
        ...
        self._publish(
            "arm.moving",
            {"target_x": tx, "target_y": ty, "target_z": tz, "status": "started"},
        )
        ...
        return {
            "status": "error",
            "message": f"Position ({tx:.0f},{ty:.0f},{tz:.0f}) unreachable",
        }
    ...
    ) -> tuple[Any, list[dict] | None, list[str] | None]:
    ...
    def _execute_task_sync(self, task: str, auto_confirm: bool) -> str:
        ...
        # Auto-capture initial frame so the LLM can see the scene immediately
        try:
            ...
            self.provider.append_images(
                messages,
                encoded_parts,
                text=f"[Initial camera frame. {text_result}]",
            )
        except Exception as e:
            logger.warning(f"Failed to capture initial frame: {e}")
