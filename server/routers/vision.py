"""Vision endpoints: look and segment."""
...
async def look(request: Request):
    """Capture a frame and encode for SAM2. Returns base64 images."""
    hw: HardwareManager = request.app.state.hardware
    if hw.camera is None or hw.sam2 is None or hw.ct is None:
        raise HTTPException(503, "Hardware not ready")

    # Capture aligned frames (reuse the agent's method)
    def _capture_and_encode():
        ...
        # Cache for subsequent segment/goto calls
    ...
    return {
        ...
        "width": 640,
        "height": 480,
        "sam2_encode_time_s": round(encode_time, 2),
    }
...
async def segment(body: SegmentRequest, request: Request):
    ...
    hw: HardwareManager = request.app.state.hardware
    if hw.sam2 is None or hw.ct is None:
        raise HTTPException(503, "Vision not ready")
